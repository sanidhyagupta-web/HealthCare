...
linear_issue: null               # set to the issue URL once a real ticket exists (live ticket_mode only)
---
...
While `ticket_mode` is `"draft"` (see `.claude/wiki-project.env`), a decision never gets a local
ticket file either — it leaves `linear_issue: null` and the would-be ticket surfaces only in that
run's summary output. There is no `triage/` directory: a local draft ticket would just duplicate
content the Decision Record itself already holds (`## Statement`, `evidence_quote`,
`reconciliation`). A decision links straight to its Linear issue (`linear_issue`) once one exists,
in `ticket_mode: "live"` — there is no local draft/triage file at any point, since that would
duplicate what Linear already tracks.
