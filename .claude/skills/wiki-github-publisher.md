...
scaffold, so wiki content lands as additional commits on one PR per project rather than a new PR
per run. The branch outlives the onboarding PR: that first PR is merged to put the `.claude/`
harness and the wiki tree on the default branch, and later runs keep pushing to the same branch,
so whichever PR is open on it at the time is the one this run's commits join.
