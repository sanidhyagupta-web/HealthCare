...
This repo has no build tool wrapper — `pytest` is run directly from the repo root (`conftest.py`
adds the root to `sys.path` so `ingestion.*`, `security.*`, etc. resolve without an installed
package):
...
pytest tests/unit/test_researcher_role.py::test_apply_role_mask_removes_patient_name
```

There is no separate integration-test runner or test container setup in this repo — `tests/unit/`
holds both business-logic (Tier 1) and cross-module contract (Tier 2) tests in the same files,
using an on-disk SQLite DB (`db/healthcare_registry.db`, see `db/database.py`) rather than
Testcontainers. No CI job currently runs `pytest` (`.github/workflows/claude-review.yml` only runs
the PR-review script) — treat `pytest` as the verification step to run locally before committing.
