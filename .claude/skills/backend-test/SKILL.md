...
```bash
# Full suite (this repo is small enough to run in full)
pytest
...
pytest tests/unit/test_bulk_ingestion.py::test_name
```

Note: this repo has no Spring/Testcontainers-style integration harness — `db/database.py` runs
against a local SQLite file via SQLAlchemy, so "Tier 2" tests here just exercise the real
SQLAlchemy session (`db.database.get_db`) instead of a containerized DB.
