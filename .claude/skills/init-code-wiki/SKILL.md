...
- **If none is returned** — this skill is the first of the two to reach this point. Create it:
  ...
  - **Merge this PR.** It is the one-time onboarding PR, not a long-lived changeset: the harness above only reaches the default branch when it lands, and the wiki tree has to be there before completed work can re-derive what the system is
  - Empty by design. Later code-wiki content arrives as its own commits from the closeout stage, not as edits to this scaffold
