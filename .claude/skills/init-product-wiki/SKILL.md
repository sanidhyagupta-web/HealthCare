...
`## Decisions`, `## Evidence` and `## Relationships` are link tables and are unaffected by this —
a `DEC-NNNN` id is a wiki reference, not a code identifier.
...
- **If none is returned** — this skill is the first of the two to reach this point. Create it:
  ...
  - Commits the .claude/ coding harness (rules and agents) this repo needs before any agent can work in it
  - **Merge this PR.** It is the one-time onboarding PR, not a long-lived changeset: the harness above only reaches the default branch when it lands, and the wiki tree has to be there for the alignment loop to have any product intent to check against
  - Empty by design. The scaffold carries no decisions or feature requests — /wiki-ingest writes those as real work arrives, and an empty backlog is the correct initial state rather than something to backfill first
