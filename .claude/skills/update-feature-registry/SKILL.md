...
description: SUPERSEDED — docs/features/ is no longer the store; the code wiki is. A code-wiki equivalent of this skill does not exist yet, so there is currently no ticket-by-ticket upkeep step. Do not run. Update docs/features/ after a ticket is completed. Updates or creates the relevant per-feature index.md files and keeps the master index in sync so future planning can identify what might break.
